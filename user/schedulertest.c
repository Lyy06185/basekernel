#include "library/syscalls.h"
#include "library/stdio.h"
#include "library/errno.h"

//scheule process according to priority and print info
//warn if failure
void schedule_process(const char *exec, int priority, int index) {
    int pfd = syscall_open_file(KNO_STDDIR, exec, 0, 0);

    if (pfd >= 0) {
        int pid = syscall_process_run_with_priority(pfd, 0, &exec, priority);
        if (pid > 0) {
            printf("=== PROCESS %d (%s) WITH PID %d and PRIORITY %d HAS BEEN SCHEDULED ===\n", index, exec, pid, priority);
        } else {
            printf("[ERROR] Failed to create process %s (Error code: %s)\n", exec, strerror(pid));
        }
    } else {
        printf("[ERROR] Failed to open file %s (Error code: %s)\n", exec, strerror(pfd));
    }
}

int main(int argc, char** argv) {
    printf("=== Scheduler test started ===\n");

    int n = 5, i;
    const char *execs[] = {"bin/process1.exe", "bin/process2.exe", "bin/process3.exe", "bin/process4.exe", "bin/process5.exe"};
    int priorities[] = {9, 7, 2, 1, 5};
    for (i = 0; i < n; i++) {
        schedule_process(execs[i], priorities[i], i+1);
    }
    printf("=== Processes will start running ===\n");
    syscall_process_run_blocked_all();
    return 0;
}